    ...
    <el-card>
        ...
        <div v-if="currentDevices.length > 0">
            ...
            >
                ...
                <el-button
                    type="primary"
                    @click.native.prevent="openPhoneService(scope.row)"
                    plain
                    v-waves
                >首先开启手机服务</el-button>
                    <el-switch @change="switchAudio(scope.row)" on-value="1" off-value="0" v-model="scope.row.enableAudio" >2.开启/关闭音频</el-switch>
                    </template>
                </el-table-column>
...
export default {
    ...
    created() {
        ...
        ipcRenderer.on('devices', (event, _devices) => {
            ...
            const devices = _devices
                ...
                .map(({ id }) => ({ id, name: this.$store.get(id) || id, method: Regular('ip', id) ? wireless : wired, enableAudio: "1", }))
    ...
    methods: {
        ...
        openPhoneService(device){
            this.$notify.info('在手机上点击开启投射屏幕' + this.$t('management.open.loading'), 2000)
            ipcRenderer.send('openPhoneService', { device }) 
        },
        switchAudio (device) {
            this.$notify.info(this.$t('management.open.loading'), 5000)
            ipcRenderer.send('switchAudio', { device })
        },
        open(devices) {
